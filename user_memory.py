...
""".strip()
    ...
    async def _review_and_rewrite_response(
        ...
        *,
        ...
        proactive_fix = self._recent_proactive_context_reply_fix(user, inbound_text, response_text)
        if proactive_fix:
            payload = self._recent_proactive_payload(user)
            logger.info(
                "[PrivateCompanion] 被动回复偏离最近主动消息,已按主动上下文纠偏: inbound=%s proactive=%s before=%s after=%s",
                _single_line(inbound_text, 120),
                _single_line(payload.get("message"), 160),
                _single_line(response_text, 160),
                _single_line(proactive_fix, 160),
            )
            return proactive_fix
...
""".strip()
    ...
    async def _format_proactive_reply_context(self, event: AstrMessageEvent) -> str:
        try:
            ...
            return ""
        inbound_text = _single_line(getattr(event, "message_str", ""), 120)
        ...
        payload = self._recent_proactive_payload(user)
        last_sent = _safe_float(payload.get("last_sent"), 0)
        last_message = _single_line(payload.get("message"), 240)
        if not last_sent or not last_message:
            return ""
        if _now_ts() - last_sent > self.proactive_reply_context_hours * 3600:
            return ""

        reply_context = user.get("last_proactive_reply_context")
        force_reply_context = False
        if isinstance(reply_context, dict):
            context_ts = _safe_float(reply_context.get("ts"), 0)
            context_text = _single_line(reply_context.get("text"), 120)
            context_sent = _safe_float(reply_context.get("proactive_sent_at"), 0)
            same_text = (
                bool(context_text)
                and bool(inbound_text)
                and self._compact_repeat_text(context_text) == self._compact_repeat_text(inbound_text)
            )
            force_reply_context = (
                context_ts > 0
                and _now_ts() - context_ts <= 10 * 60
                and (not context_sent or abs(context_sent - last_sent) <= 3)
                and (same_text or self._inbound_is_low_information_proactive_reply(inbound_text))
            )

        return self._build_proactive_reply_core(
            user,
            inbound_text=inbound_text,
            force_reply=force_reply_context,
        )
...
""".strip()
    ...
    def _recent_proactive_payload(self, user: dict[str, Any]) -> dict[str, Any]:
        if not isinstance(user, dict):
            return {}
        last_sent = _safe_float(user.get("last_proactive_sent_at"), 0) or _safe_float(user.get("last_sent"), 0)
        last_message = _single_line(user.get("last_proactive_message"), 500) or _single_line(user.get("last_companion_message"), 500)
        continuity = user.get("state_continuity") if isinstance(user.get("state_continuity"), dict) else {}
        continuity_ts = _safe_float(continuity.get("last_action_ts"), 0) if isinstance(continuity, dict) else 0
        continuity_text = _single_line(continuity.get("last_action_text"), 500) if isinstance(continuity, dict) else ""
        if continuity_text and continuity_ts and (not last_sent or continuity_ts >= last_sent - 3):
            last_sent = continuity_ts
            last_message = continuity_text
        actions = user.get("action_consequences")
        if isinstance(actions, list):
            for item in reversed(actions[-6:]):
                if not isinstance(item, dict):
                    continue
                item_ts = _safe_float(item.get("ts"), 0)
                item_text = _single_line(item.get("text"), 500)
                if item_text and item_ts and (not last_sent or item_ts >= last_sent - 3):
                    last_sent = item_ts
                    last_message = item_text
                    break
        if not last_sent or not last_message:
            return {}
        if _now_ts() - last_sent > self.proactive_reply_context_hours * 3600:
        ...
        return {
            ...
            "reason": _single_line(user.get("last_proactive_reason"), 80),
            "action": _single_line(user.get("last_proactive_action"), 40),
            "behavior": _single_line(user.get("last_proactive_behavior_summary"), 120),
            "motive": _single_line(user.get("last_proactive_motive"), 160),
        }

    def _mark_recent_proactive_reply_turn(
        self,
        user: dict[str, Any],
        inbound_text: str,
        *,
        ...
        """Remember that the current inbound turn is replying to the last proactive send."""
        if not isinstance(user, dict):
            return False
        payload = self._recent_proactive_payload(user)
        last_sent = _safe_float(payload.get("last_sent"), 0)
        last_message = _single_line(payload.get("message"), 500)
        if not last_sent or not last_message:
            return False
        now_ts = now or _now_ts()
        if now_ts - last_sent > self.proactive_reply_context_hours * 3600:
            return False
        user["last_proactive_reply_context"] = {
            "ts": now_ts,
            "text": _single_line(inbound_text, 220),
            "proactive_sent_at": last_sent,
            "message": last_message,
            "reason": _single_line(payload.get("reason"), 80),
            "action": _single_line(payload.get("action"), 40),
            "behavior": _single_line(payload.get("behavior"), 120),
            "motive": _single_line(payload.get("motive"), 160),
        }
        return True

    def _inbound_asks_recent_proactive_context(self, inbound_text: str) -> bool:
        compact = self._compact_repeat_text(inbound_text)
        if not compact:
            return False
        if re.search(r"(你刚|刚才|刚刚|刚说|你说的|你刚说的|上条|上一句)", compact) and re.search(r"(什么|啥|怎么|发生|哪件|哪个|指的)", compact):
            return True
        return bool(re.search(r"(发生什么了|发生啥了|怎么了|咋了|啥情况|什么情况|咋回事|怎么回事|出什么事|什么事|被什么吓|为啥吓)", compact))

    def _inbound_is_low_information_proactive_reply(self, inbound_text: str) -> bool:
        raw = _single_line(inbound_text, 80)
        compact = self._compact_repeat_text(raw)
        if not compact:
            return False
        if self._inbound_asks_recent_proactive_context(raw):
            return True
        if len(compact) <= 10 and any(token in raw for token in ("？", "?", "嗯", "哦", "啊", "诶", "欸")):
            return True
        return compact in {
            "什么",
            "啥",
    ...
    def _salient_terms_for_proactive_anchor(self, text: str) -> list[str]:
        cleaned = _single_line(text, 500)
        if not cleaned:
            return []
        preferred = [
            "汽车警报", "警报", "楼下", "吓了一跳", "吓到", "突发", "小状况",
            "三花猫", "小猫", "薄荷糖", "便签", "水杯", "数学题", "物理课",
            "群里", "说说", "评论", "本子", "漫画", "视频", "下雨", "天色",
        ]
        terms = [term for term in preferred if term in cleaned]
        rough_parts = re.split(
            r"(?:你呢|我呢|今天|刚刚|刚才|刚|有|没有|什么|怎么|一下|一点|那个|这个|就是|不是|被|把|的|了|啦|吗|嘛|呢|呀|啊|哦|[，,。.!！?？；;：:\s]+)",
            cleaned,
        )
        for part in rough_parts:
            part = part.strip()
            if 2 <= len(part) <= 8 and part not in {"用户", "主动", "消息", "事情", "小事"} and part not in terms:
                terms.append(part)
            if len(terms) >= 10:
                break
        return terms[:10]

    def _reply_drifts_from_recent_proactive(self, response_text: str, proactive_message: str) -> bool:
        response = _single_line(response_text, 800)
        if not response or not proactive_message:
            return False
        terms = self._salient_terms_for_proactive_anchor(proactive_message)
        overlap = [term for term in terms if term and term in response]
        stale_markers = (
            "凌晨", "喊我", "叫醒", "睡着", "不理你", "道歉", "回得慢", "半拍",
            "刚洗漱", "早安", "昨晚", "昨天", "之前那次",
        )
        has_stale_marker = any(marker in response for marker in stale_markers if marker not in proactive_message)
        if has_stale_marker and not overlap:
            return True
        return bool(terms and not overlap and len(response) >= 18)

    def _fallback_recent_proactive_context_reply(self, proactive_message: str) -> str:
        message = _single_line(proactive_message, 220)
        if not message:
            return ""
        if "汽车警报" in message or ("警报" in message and "楼下" in message):
            return "就是楼下车的警报突然响了一下，我被吓到而已。"
        if "印象深刻" in message:
            return "我刚才是想问你最近有没有什么印象深刻的互动或事情，不是说我这边发生了什么。"
        head = re.split(r"(?:[，,；;。.!！?？]\s*)?你呢|[？?]", message, maxsplit=1)[0].strip()
        head = _single_line(head, 90)
        if not head:
            return ""
        if not head.startswith(("我", "刚", "就是")):
            head = "刚才那句“" + head + "”"
        return f"就是{head}，没别的。"

    def _recent_proactive_context_reply_fix(self, user: dict[str, Any], inbound_text: str, response_text: str) -> str:
        context_dependent = self._inbound_asks_recent_proactive_context(
            inbound_text
        ) or self._inbound_is_low_information_proactive_reply(inbound_text)
        if not context_dependent:
            return ""
        payload = self._recent_proactive_payload(user)
        proactive_message = _single_line(payload.get("message"), 500)
        if not proactive_message:
            return ""
        if not self._reply_drifts_from_recent_proactive(response_text, proactive_message):
            return ""
        return self._fallback_recent_proactive_context_reply(proactive_message)
    ...
    def _response_review_flags(self, text: str, user: dict[str, Any], *, inbound_text: str = "") -> list[str]:
        ...
        proactive_fix = self._recent_proactive_context_reply_fix(user, inbound_text, cleaned)
        if proactive_fix:
            flags.append("drifts_from_recent_proactive")
...
""".strip()
    ...
    def _build_proactive_reply_core(
        self,
        user: dict[str, Any],
        *,
        inbound_text: str = "",
        ...
        if inbound:
            lines.append(f"本轮用户消息：“{inbound}”。")
        if keywords:
            lines.append(f"最近主动消息关键词：{keywords}")
        strong_anchor = force_reply or self._inbound_is_low_information_proactive_reply(inbound)
        if strong_anchor:
            lines.extend(
                [
                    "本轮私聊可能没有保留原生上一轮历史；这段就是必要的短期承接上下文。",
                    "如果用户当前只是“？”“怎么了”“刚才说啥”这类低信息回应,先把它理解为在回应/询问上一条主动消息,不要改接天气、日程、旧记忆或新的生活状态。",
                    "上一条主动消息已经真实发出过；不要复述一遍重新发问,而是用一两句自然解释刚才那句话想接什么。",
                ]
            )
        else:
            lines.extend(

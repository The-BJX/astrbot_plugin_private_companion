...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：5.6.7 深化 RememberYou / MemoryCompanion 联动，补强情绪漂移、梦境碎片、未完成话题和跨窗口情绪连续性。
...
- 用途：接入外部长期记忆插件，让陪伴状态、日程、主动动机、群聊观察和长期记忆互相补位。
- RememberYou / MemoryCompanion：`astrbot_plugin_remember_you` / `astrbot_plugin_memory_companion`，偏陪伴型记忆编排，负责长期记忆注入、私聊/群聊权限边界、Bot 自我时间线、未完成话题、情绪漂移和 LivingMemory 旧库迁移。
- LivingMemory：<https://github.com/lxfight-s-Astrbot-Plugins/astrbot_plugin_livingmemory>，偏通用长期记忆引擎，提供大规模长期记忆、向量检索、图谱记忆和 `recall_long_term_memory` 工具。
- 相关设置：LivingMemory 联动、长期记忆召回工具名；RememberYou / MemoryCompanion 的桥接开关在记忆插件侧配置。

检测到 RememberYou / MemoryCompanion 后，本插件会把真实主动消息、日程片段、梦境碎片、当前状态、主动动机、群聊发言者和关系线索等结构化信息交给记忆插件整理；记忆插件再统一处理长期沉淀、权限隔离、召回注入和调试日志。本插件则继续负责生活状态、主动节奏、当前轮表达、图片/语音/识屏等外部动作。

对陪伴场景来说，RememberYou / MemoryCompanion 的优势不只是“多一个记忆库”。它会把私聊记忆、群聊公共记忆、Bot 自我时间线、共享记忆和内部记录分开管理，按当前窗口、权限和话题相关性临时注入；还能区分“可以自然明说”“只影响语气”“除非用户问起否则不要提”的记忆用法。这样 Bot 更容易做到记得住、想得起、不过度翻旧账，也不把私聊内容带进群聊。

如果已经使用过 LivingMemory，建议先在 RememberYou / MemoryCompanion 里执行迁移预览，再确认导入：
...
迁移会优先导入 LivingMemory 的完整 `documents` 摘要，跳过派生碎片、原子事实和不完整行；导入前可自动备份当前记忆库，导入后会按私聊/群聊归属放入对应可见范围。导入完成后，建议先打开记忆插件的注入调试日志或拓展页检查召回效果，再决定是否关闭 LivingMemory 的自动注入，避免同一轮回复被两套长期记忆重复影响。

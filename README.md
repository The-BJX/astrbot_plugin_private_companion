...
7. 如果保留普通被动增强，建议让 `passive_injection_position` 使用默认的尾部/缓存友好策略。它会把变化较大的状态片段放到请求后部，减少破坏主模型缓存前缀；只有确实需要更强约束时再改成 system 注入。
...
- `target_user_ids` 只负责启动时预热私聊对象；真正的用户关系、主动额度和角色身份建议到“私聊”页检查。
- `PLUGIN_VISION_PROVIDER_ID` 是插件识图模型，负责图片理解、私密阅读读后感和部分视觉摘要；`NARRATION_PROVIDER_ID` 只是工具结果转述兜底，不适合作为主要读图模型。
...
- 标签链路：插件提示词会优先要求 LLM 使用 `<pc_tts>...</pc_tts>`，发送前再交给插件统一转成语音组件。快速标签规则会按目标语种生成示例：中文语音直接中文朗读，日语/英语语音块后补对应中文含义；若当前 TTS provider 支持情绪标签，规则和示例会按语种给出对应标签，例如中文用 `[开心]`，日语用 `[嬉しい]`，英语用 `[happy]`。这样可以避免 AstrBot 原生 `<tts>` 装饰器抢先处理，导致插件的语种修正、翻译和中文释义补全失效；历史或模型自行输出的 `<tts>` 仍会尽量兼容处理。

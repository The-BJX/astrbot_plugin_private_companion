...
如果只想使用“Bot 偶尔主动来找我”的能力，不希望本插件参与普通私聊/群聊回复，可以开启 `enable_proactive_only_mode`（主动消息专用模式）。开启后，插件仍保留日程、状态、主动意愿和私聊主动发送；普通私聊/群聊不会注入本插件提示词，也不会触发图片/转发处理、防抖、群聊观察、回复规划/自检、隐藏预约、TTS 后处理、分段发送或插件 LLM 工具。用户回复主动消息时仍会被轻量记录为已回应，用于避免误判为持续未回复。这个模式会显著减少普通被动回复中的动态提示词变化，让主模型 prompt 更稳定，更容易命中服务端缓存。扩展页会自动锁定被该模式覆盖的普通被动功能，但只是在 UI 上标记为“被运行模式覆盖”，不会改写原配置；关闭该模式后原开关继续按保存值生效。

如果仍需要普通被动陪伴增强，但发现服务端 prompt 缓存命中明显下降，可以调整 `passive_injection_position`。默认 `prompt` 会把每轮变化的动态片段尽量追加到当前请求末尾，避免插入 `system_prompt` 后破坏人设和历史上下文的缓存前缀；选择 `system_prompt` 时约束更强，但更容易降低缓存命中率；`auto` 当前按缓存优先处理。

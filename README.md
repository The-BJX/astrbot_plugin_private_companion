...
- TTS 频率：`tts_frequency_control_mode=global` 时使用“间隔 + 概率”的新版频控。全局概率会作为提示词层面的方向约束：概率未命中时提示模型默认不要输出 `<tts>`，但如果用户明确想听语音或模型已经合理输出 `<tts>`，不会再被概率硬剥离；最终发送仍受 TTS provider 可用性和会话最小间隔保护。

...
- 扩展页管理：在 AstrBot WebUI 中查看和管理私聊、群聊、关系网、状态、梦境、书柜、主动计划、功能开关、模型配置、Token 消耗和排障结果；模型页会按用途分组展示 Provider、回退关系、当前使用项和测试入口。
...
- 用途：支持主动短语音，并兼容插件私有 `<pc_tts>...</pc_tts>`、旧式 `<tts>...</tts>`、日语、双语或特殊 TTS 人格规则。
- 配置边界：插件里的“主动语音文案模型”和“TTS 后处理模型”都是文本模型，只负责生成/修正语音文案和语音标记；真正把文本合成音频的是 AstrBot 当前会话的 TTS provider。阿里云百炼/CosyVoice 等语音合成模型需要在 AstrBot 的 TTS 配置中启用，本插件会读取当前会话的 TTS provider。
...
- 生成路径：“快速标签”是低延迟路径，主模型可按规则写插件私有 `<pc_tts>`，插件发送前统一清洗、翻译和合成；“稳定后处理”路径下，主模型只写普通回复，发送前由 TTS 后处理模型判断是否需要语音，并完成目标语种改写。
- 人格保持：TTS 后处理模型执行翻译、语种修正和中文释义补全时，会读取当前 AstrBot 人格的轻量语音风格参考，用于保留称呼、距离感、口癖和角色边界；不会把人格设定直接复述进消息。
- 后处理判断：稳定后处理模式会把用户本轮原话、插件规则快判线索和自动语音概率线索一起交给后处理模型，由模型判断用户是否要求/期待语音以及哪一小段适合转成语音；规则线索只作参考，不再直接把“是否语音”定死。
- TTS 频率：“新版全局频控”使用“间隔 + 概率”。全局概率会作为提示词层面的方向约束：概率未命中时会明确要求模型在没有用户语音请求时必须纯文字，不主动输出 `<pc_tts>`、`<tts>` 或等价语音内容；弱约束下如果用户明确想听语音或模型仍合理输出语音标签，不会再被概率硬剥离，最终发送仍受 TTS provider 可用性和会话最小间隔保护。
- TTS 约束强度：仅“快速标签”路径使用。默认弱约束保持“提示词引导 + LLM 判断”为主；强约束会在全局频控概率未命中或会话冷却内注入“本轮禁止语音”的反向提示词，并在发送前阻止 LLM 已写出的 `<pc_tts>/<tts>` 和快速标签自动语音。稳定后处理路径不使用该项，概率只作为后处理模型的保守线索，会话间隔仍决定是否实际发送语音。
- 私聊/群聊覆盖：可分别设置私聊/群聊的语音触发概率和最小间隔；概率字段在扩展页按百分比 `0-100` 填写，填 `-1` 表示继承全局，适合把群聊设得更低频、更长间隔。
- 旧版行为：“旧版频控”按旧路径逻辑触发，主要交给 LLM 判断是否“适合采用”语音，不使用新版的全局概率/间隔提示约束。
...
- 可选本机联动：开启“TTS 本机播放”后，TTS 生成的音频会在运行 AstrBot 的电脑上播放；开启“直播字幕同步”后，会同步推送朗读文本到“我会直播圈米养你”的打字机字幕 overlay，默认地址为 `http://127.0.0.1:18081/show`。
- 相关设置：主动语音、语音字数上限、TTS 语音增强、生成路径、目标语种、TTS 后处理模型、额外语音规则、频率控制、约束强度、会话最小间隔、触发概率、私聊/群聊覆盖、本机播放、直播字幕同步。
